# Class taken for the module ChangeTracker from some minor changes.
# The original file can be found here: https://github.com/fedorov/ChangeTrackerPy/blob/master
...
class Helper( object ):
  ...
  def Error( message ):
    ...
    print "[MRIChangeDetector " + time.strftime( "%m/%d/%Y %H:%M:%S" ) + "]: ERROR: " + str( message )
    sys.stdout.flush()
  ...
  def Debug( message ):
    ...
    if showDebugOutput:
        print "[MRIChangeDetector " + time.strftime( "%m/%d/%Y %H:%M:%S" ) + "] DEBUG: " + str( message )
        sys.stdout.flush()
